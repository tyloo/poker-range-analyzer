import { PositionRange } from '../types';

// BTN (Button) RFI Range - Widest opening range
// ~45% of hands
export const btnRange: PositionRange = {
  // All pairs
  'AA': { raise: 100, call: 0, fold: 0 },
  'KK': { raise: 100, call: 0, fold: 0 },
  'QQ': { raise: 100, call: 0, fold: 0 },
  'JJ': { raise: 100, call: 0, fold: 0 },
  'TT': { raise: 100, call: 0, fold: 0 },
  '99': { raise: 100, call: 0, fold: 0 },
  '88': { raise: 100, call: 0, fold: 0 },
  '77': { raise: 100, call: 0, fold: 0 },
  '66': { raise: 100, call: 0, fold: 0 },
  '55': { raise: 100, call: 0, fold: 0 },
  '44': { raise: 100, call: 0, fold: 0 },
  '33': { raise: 100, call: 0, fold: 0 },
  '22': { raise: 100, call: 0, fold: 0 },

  // All suited aces
  'AKs': { raise: 100, call: 0, fold: 0 },
  'AQs': { raise: 100, call: 0, fold: 0 },
  'AJs': { raise: 100, call: 0, fold: 0 },
  'ATs': { raise: 100, call: 0, fold: 0 },
  'A9s': { raise: 100, call: 0, fold: 0 },
  'A8s': { raise: 100, call: 0, fold: 0 },
  'A7s': { raise: 100, call: 0, fold: 0 },
  'A6s': { raise: 100, call: 0, fold: 0 },
  'A5s': { raise: 100, call: 0, fold: 0 },
  'A4s': { raise: 100, call: 0, fold: 0 },
  'A3s': { raise: 100, call: 0, fold: 0 },
  'A2s': { raise: 100, call: 0, fold: 0 },

  // Suited kings
  'KQs': { raise: 100, call: 0, fold: 0 },
  'KJs': { raise: 100, call: 0, fold: 0 },
  'KTs': { raise: 100, call: 0, fold: 0 },
  'K9s': { raise: 100, call: 0, fold: 0 },
  'K8s': { raise: 100, call: 0, fold: 0 },
  'K7s': { raise: 100, call: 0, fold: 0 },
  'K6s': { raise: 100, call: 0, fold: 0 },
  'K5s': { raise: 100, call: 0, fold: 0 },
  'K4s': { raise: 75, call: 0, fold: 25 },
  'K3s': { raise: 75, call: 0, fold: 25 },
  'K2s': { raise: 50, call: 0, fold: 50 },

  // Suited queens
  'QJs': { raise: 100, call: 0, fold: 0 },
  'QTs': { raise: 100, call: 0, fold: 0 },
  'Q9s': { raise: 100, call: 0, fold: 0 },
  'Q8s': { raise: 100, call: 0, fold: 0 },
  'Q7s': { raise: 75, call: 0, fold: 25 },
  'Q6s': { raise: 75, call: 0, fold: 25 },
  'Q5s': { raise: 50, call: 0, fold: 50 },
  'Q4s': { raise: 50, call: 0, fold: 50 },

  // Suited jacks
  'JTs': { raise: 100, call: 0, fold: 0 },
  'J9s': { raise: 100, call: 0, fold: 0 },
  'J8s': { raise: 100, call: 0, fold: 0 },
  'J7s': { raise: 75, call: 0, fold: 25 },
  'J6s': { raise: 50, call: 0, fold: 50 },

  // Suited connectors and gappers
  'T9s': { raise: 100, call: 0, fold: 0 },
  'T8s': { raise: 100, call: 0, fold: 0 },
  'T7s': { raise: 75, call: 0, fold: 25 },
  '98s': { raise: 100, call: 0, fold: 0 },
  '97s': { raise: 100, call: 0, fold: 0 },
  '96s': { raise: 75, call: 0, fold: 25 },
  '87s': { raise: 100, call: 0, fold: 0 },
  '86s': { raise: 100, call: 0, fold: 0 },
  '85s': { raise: 50, call: 0, fold: 50 },
  '76s': { raise: 100, call: 0, fold: 0 },
  '75s': { raise: 100, call: 0, fold: 0 },
  '74s': { raise: 50, call: 0, fold: 50 },
  '65s': { raise: 100, call: 0, fold: 0 },
  '64s': { raise: 75, call: 0, fold: 25 },
  '54s': { raise: 100, call: 0, fold: 0 },
  '53s': { raise: 75, call: 0, fold: 25 },
  '43s': { raise: 75, call: 0, fold: 25 },
  '32s': { raise: 50, call: 0, fold: 50 },

  // Offsuit broadway
  'AKo': { raise: 100, call: 0, fold: 0 },
  'AQo': { raise: 100, call: 0, fold: 0 },
  'AJo': { raise: 100, call: 0, fold: 0 },
  'ATo': { raise: 100, call: 0, fold: 0 },
  'A9o': { raise: 100, call: 0, fold: 0 },
  'A8o': { raise: 100, call: 0, fold: 0 },
  'A7o': { raise: 100, call: 0, fold: 0 },
  'A6o': { raise: 100, call: 0, fold: 0 },
  'A5o': { raise: 100, call: 0, fold: 0 },
  'A4o': { raise: 100, call: 0, fold: 0 },
  'A3o': { raise: 75, call: 0, fold: 25 },
  'A2o': { raise: 75, call: 0, fold: 25 },
  'KQo': { raise: 100, call: 0, fold: 0 },
  'KJo': { raise: 100, call: 0, fold: 0 },
  'KTo': { raise: 100, call: 0, fold: 0 },
  'K9o': { raise: 100, call: 0, fold: 0 },
  'K8o': { raise: 75, call: 0, fold: 25 },
  'K7o': { raise: 50, call: 0, fold: 50 },
  'QJo': { raise: 100, call: 0, fold: 0 },
  'QTo': { raise: 100, call: 0, fold: 0 },
  'Q9o': { raise: 100, call: 0, fold: 0 },
  'Q8o': { raise: 50, call: 0, fold: 50 },
  'JTo': { raise: 100, call: 0, fold: 0 },
  'J9o': { raise: 100, call: 0, fold: 0 },
  'J8o': { raise: 50, call: 0, fold: 50 },
  'T9o': { raise: 100, call: 0, fold: 0 },
  'T8o': { raise: 75, call: 0, fold: 25 },
  '98o': { raise: 100, call: 0, fold: 0 },
  '97o': { raise: 50, call: 0, fold: 50 },
  '87o': { raise: 100, call: 0, fold: 0 },
  '76o': { raise: 75, call: 0, fold: 25 },
  '65o': { raise: 50, call: 0, fold: 50 },
};
